//var a = 5;
function ItoPconveter_reverse(inputValue,OutputValue){	
		var w = 900;
    var h = 500;
...
var width = $(window).width();
 ...
 x = 50;
	...
	var IPNozzel = Nozzelanim_r (x, y,a,OutputValue);
	var IPFlapper = FlapperAnim_r (x, y ,a);
	var IPspringscrew = springscrew_r (x,y,a);
	var IPplusNozzel = plusNozzel_r(x,y,a);
	var IPminusNozzel = minusNozzel_r(x,y,a);
	var IPOtherParts = OtherParts_r(x,y,inputValue);
	
};


function Nozzelanim_r (x ,y , a, OutputValue)
{
	x = x + a;
	y = y + a;
...
//	paper.path('M'+(x+70-a)+' '+(y+220)+' l 0 7 l 30 0 0 -7 z' ).attr({'fill': '#666','stroke':'black', 'stroke-width':'2'});
	...
	var nozzelScrew = paper.path('M'+(x+160)+' '+(y-10-a)+' l -7 0 0 37 7 0 0 -15 50 0 0 -7 -50 0 0 -15' ).attr({'fill': '#666','stroke':'black', 'stroke-width':'2'}).toFront();
//	leftnozzel.rotate(a);
//	rightnozzel.rotate(a);
};

function plusNozzel_r(x,y,a)
{
	x = x + a;
	y = y + a;
...
//	paper.ellipse(x+390, y-150, 30, 15).attr({ 'fill': '#fff','stroke':'black', 'stroke-width':'3' });
	...
	//spring screw start
	paper.path('M'+(x+280)+' '+(y-170+a)+' l  0 -35 15 0 0 -7 -37 0 0 7 15 0 0 35 z' ).attr({'fill': '#666','stroke':'black', 'stroke-width':'3'}).toBack();
...
function OtherParts_r (x,y,inputValue)
{
	...
	paper.text((x+200), (y+250), "Signal pressure (0.2-1 kg/cm²)").attr({
		stroke : 'black',
		'font-size' : 15,
		"font-family": "sans-serif" 
	});
